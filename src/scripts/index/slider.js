...
(function ($) {
    ...
    $.fn.slider = function (config) {
        ...
        if (config.maxWidth) {
            b.css({maxWidth: config.maxWidth});
        }
        if (config.minWidth) {
            b.css({mixWidth: config.minWidth});
        }
        if (config.maxHeight) {
            b.css({maxHeight: config.maxHeight});
        }
        if (config.minHeight) {
            b.css({minHeight: config.minHeight});
        }
        ...
        b.on("touchstart", function (e) {
            ...
            var et = e.touches[0];
            if (m.canmove && Math.abs(et.pageY - m.initY) / Math.abs(et.pageX - m.initX) < 0.3) {
                e.preventDefault();
